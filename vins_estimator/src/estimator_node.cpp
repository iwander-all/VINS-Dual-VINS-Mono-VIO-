...
void processVIO(sensor_msgs::PointCloudConstPtr& img_msg)
{
    ...
    //assert(count_left == image.size());
    //ROS_INFO("the number in left: %d",count_left);
    //ROS_INFO("the number in right: %d",count_right);
    //ROS_INFO("the number of features: %d",image.size());
