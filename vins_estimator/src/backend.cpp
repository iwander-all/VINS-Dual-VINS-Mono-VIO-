...
void Backend::getInverseDepthInRight(Estimator *estimator,const double para_Feature[][SIZE_FEATURE])
{
    int i = 0;
    for (auto &it_per_id : estimator->f_manager.feature)
    {
        ...
        if (!(it_per_id.used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 2))
            continue;
        Vector3d un_pts = it_per_id.feature_per_frame[0].point;
        Vector3d un_pts1 = it_per_id.feature_per_frame1[0].point;
        Vector3d A = it_per_id.estimated_depth * estimator->ric[0] * un_pts + estimator->tic[0] - estimator->tic[1];
        Vector3d b = estimator->ric[1]*un_pts1;
        double s2_1 = b[0] / A[0];
        double s2_2 = b[1] / A[1];
        para_Feature1[i][0] =(s2_1 + s2_2) / 2;
        i++;
    }
    //assert(i == estimator->f_manager.getFeatureCount());
    //printf("getInverseDepthInRight:",para_Feature1);
    //printf("getInverseDepthInLeft:",para_Feature);
}
...
void Backend::vector2double(Estimator *estimator)
{
    ...
    int feature_count = estimator->f_manager.getFeatureCount();
    ...
    //getInverseDepthInRight(estimator,para_Feature);
...
void Backend::double2vector(Estimator *estimator)
{
    ...
    //ROS_INFO("depth in left: ", dep);

    VectorXd dep1 = estimator->f_manager.getDepthVectorInRight();
    for (int i = 0; i < feature_count; i++)
        dep1(i) = para_Feature1[i][0];
    estimator->f_manager.setDepthInRight(dep1);
    //ROS_INFO("depth in right: ", dep1);
...
void Backend::nonLinearOptimization(Estimator *estimator,ceres::Problem &problem,ceres::LossFunction *loss_function)
{
    ...
    if (OPTIMIZATION_PATTERN == 2)
    {
        for (auto &it_per_id : estimator->f_manager.feature)
        {
            ...
            int imu_i = it_per_id.start_frame1, imu_j = imu_i - 1;
            
            Vector3d pts_i = it_per_id.feature_per_frame1[0].point;
            ...
            for (auto &it_per_frame : it_per_id.feature_per_frame1)
            {
                ...
                if (ESTIMATE_TD)
                {
                        ProjectionTdFactor *f_td = new ProjectionTdFactor(pts_i, pts_j, it_per_id.feature_per_frame1[0].velocity, it_per_frame.velocity,
                                                                        it_per_id.feature_per_frame1[0].cur_td, it_per_frame.cur_td,
                                                                        it_per_id.feature_per_frame1[0].uv.y(), it_per_frame.uv.y());
                        problem.AddResidualBlock(f_td, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[1], para_Feature1[feature_index], para_Td[0]);
                }
                else
                {
                    ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j);
                    problem.AddResidualBlock(f, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[1], para_Feature1[feature_index]);
                }
                f_m_cnt++;
            }
        }
    ...
    // reprojection error in left nad right roundly
    if (OPTIMIZATION_PATTERN == 1)
    {
        int camera_choice = 0;
        int right_count = 0;
        int left_count = 0;
        ...
        //ROS_INFO("in Optimization,right: %d",right_count);
        //ROS_INFO("in Optimization,left: %d",left_count);
        //ROS_INFO("feature_index: %d",feature_index);
        //ROS_INFO("camera_choice: %d",camera_choice);
    }
...
void Backend::margOld(Estimator *estimator,ceres::LossFunction *loss_function)
{
            ...
            for (auto &it_per_frame : it_per_id.feature_per_frame1)
            {
                ...
                if (ESTIMATE_TD)
                {
                    ProjectionTdFactor *f_td = new ProjectionTdFactor(pts_i, pts_j, it_per_id.feature_per_frame1[0].velocity, it_per_frame.velocity,
                                                                      ...
                                                                      it_per_id.feature_per_frame1[0].uv.y(), it_per_frame.uv.y());
                    ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(f_td, loss_function,
                                                                                   vector<double *>{para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[1], para_Feature1[feature_index], para_Td[0]},
                                                                                   vector<int>{0, 3});
                    marginalization_info->addResidualBlockInfo(residual_block_info);
                }
                else
                {
                    ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j);
                    ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(f, loss_function,
                                                                                   vector<double *>{para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[1], para_Feature1[feature_index]},
                                                                                   vector<int>{0, 3});
                    marginalization_info->addResidualBlockInfo(residual_block_info);
                }
            }
    ...
    // reprojection error in left and right
    if (OPTIMIZATION_PATTERN == 1) 
    {
        int camera_choice = 0;
        int feature_index = -1;
        ...
        //ROS_INFO("in margold,right: %f",right_count);
        //ROS_INFO("in margold,left: %f",left_count);
    }
...
void Backend::slideWindowOld(Estimator *estimator)
{
    ...
    if (shift_depth)
    {
        ...
        Matrix3d R0r, R1r;
        Vector3d P0r, P1r;
        R0r = estimator->back_R0 * estimator->ric[1];
        R1r = estimator->Rs[0] * estimator->ric[1];
        P0r = estimator->back_P0 + estimator->back_R0 * estimator->tic[1];
        P1r = estimator->Ps[0] + estimator->Rs[0] * estimator->tic[1];
        estimator->f_manager.removeBackShiftDepth(R0, P0, R1, P1, R0r, P0r, R1r, P1r);
    }
    else
